...
主线程创建了一组工作者线程，然后他进入了无线的服务器循环，接受连接请求，并将得到的已连接描述符插入到缓冲区`sbuf`中，每个工作者线程的行为都非常简单，他等待直到他能从缓冲区中取出一个已连接描述符，然后调用`echo_cnt`函数回送客户端输入。

##

## 使用线程提供并行性

大多数现代机器具有多核处理器，并发程序通常在这样的机器得更快，因为操作系统内核在多个核上并行地调度这些并发线程，而不是在单个核上顺序地调度

![alt text](image-3.png)

所有程序地集合能够被划分成不相交地顺序程序集合和并发程序集合。写顺序程序只有一条逻辑流。写并发程序有多条并发流。并行程序是一个运行在多个处理器上地并发程序。

并行编程地一项重要教训：同步开销巨大，要尽可能避免。如果无可避免，必须要用尽尽可能多的有用地计算弥补这个开销。
...
一个函数被称为线程安全，当且仅当被多个并发程序反复地调用时，他一直产生正确的结果。如果一个函数不是线程安全的，我们就说他是线程不安全的。
...
有一类重要的线程安全函数，叫做可重入函数，其特点在于它们具有这样一种属性：当它们被多个线程调用时，不会引用任何共享数据。

所有函数的集合被划分成不相交的线程安全和线程不安全函数集合。可重入函数集合是线程安全函数的一个真子集。

![alt text](image-4.png)

我们总是使用术语可重入的既包括显式的可重入函数，也包括隐式的可重入函数。显式的可重入函数是那些不引用任何共享数据的函数。隐式的可重入函数是那些引用共享数据，但是在每次调用时都能保证正确性的函数。
...
当一个程序的正确性依赖于一个线程要在另一个线程到达`y`点之前到达它的控制流中的`x`点时，我们说这个程序有一个竞争条件.

### 死锁

信号量引入了一种潜在的令人厌恶的运行时错误，叫做死锁，它是指一组线程被阻塞了，等待一个永远也不会为真的条件.

![alt text](image-5.png)

死锁的重要知识

- 程序员使用P和V操作顺序不当，至于两个信号量的禁止区域重叠
