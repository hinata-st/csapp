...
#define OFFSET(ptr) ((unsigned)((char *)(ptr) - (char *)mem_heap_lo()))
#define PTR_FROM_OFFSET(off) ((char *)mem_heap_lo() + (off))

#define PREV_NODE(bp) (PTR_FROM_OFFSET(*(unsigned *)(bp)))
#define NEXT_NODE(bp) (PTR_FROM_OFFSET(*(unsigned *)((char *)(bp) + WSIZE)))

#define SET_NODE_PREV(bp, val) (*(unsigned *)(bp) = ((val) ? OFFSET(val) : 0))
#define SET_NODE_NEXT(bp, val) (*(unsigned *)((char *)(bp) + WSIZE) = ((val) ? OFFSET(val) : 0))
...
void *mm_realloc(void *ptr, size_t size)
{
    ...
    // 计算旧块的有效负载大小
    old_payload_size = GET_SIZE(HDRP(ptr)) - WSIZE;

    // 确定要拷贝的数据大小
    copy_size = MIN(old_payload_size, size);
